...
# Define server logic required to show the expected output
server <- function(input, output, session) {
  ...
  corplot <- eventReactive(input$corplot,{
    ...
    corrplot(graph, method = "circle", addgrid.col = T,type = 'upper', addCoef.col = T, 
             number.cex = .7, diag = T , tl.cex = .9)
  })
